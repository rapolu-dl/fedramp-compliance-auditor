...
TEST_CASES_DIR = os.getenv("VALIDATION_CASES_DIR", "test_cases_validation_v2")
OUTPUT_CSV = os.getenv("VALIDATION_OUTPUT_CSV", "validation_results_v2.csv")
...
def main():
    ...
    print(f"Running {len(cases)} V2 validation cases from {TEST_CASES_DIR}/")
    ...
    for i, case in enumerate(cases, 1):
        ...
        risk_ok += int(r)
        exact += int(e)
        status_confusion[(case["expected_status"], result.overall_status)] += 1
        risk_confusion[(case["expected_risk"], result.overall_risk_score)] += 1
    ...
    false_compliant = sum(
        1 for row in rows
        if row["actual_status"] == "COMPLIANT" and row["expected_status"] != "COMPLIANT"
    )
    expected_critical = sum(1 for row in rows if row["expected_risk"] == "CRITICAL")
    critical_correct = sum(
        1 for row in rows
        if row["expected_risk"] == "CRITICAL" and row["actual_risk"] == "CRITICAL"
    )
    ...
    print(f"False COMPLIANT:        {false_compliant}")
    if expected_critical:
        print(f"CRITICAL recall:        {critical_correct}/{expected_critical} = {100*critical_correct/expected_critical:.1f}%")
    print(f"Average latency:        {total_latency/n:.2f}s")
    print(f"Results CSV:            {OUTPUT_CSV}")
    print("=======================================================")

    print("\nStatus confusion:")
    for (expected, actual), count in sorted(status_confusion.items()):
        print(f"  expected={expected:22s} actual={actual:22s} count={count}")

    print("\nRisk confusion:")
    for (expected, actual), count in sorted(risk_confusion.items()):
        print(f"  expected={expected:10s} actual={actual:10s} count={count}")
